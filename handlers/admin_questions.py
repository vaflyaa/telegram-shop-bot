...
from states.questions_state import AnswerState
from handlers.menu import questions
from loader import dp, db, bot
from filters import IsAdmin

question_cb = CallbackData('question', 'chat_id', 'action')
...
async def process_questions(message: Message):
    ...
    if len(questions) == 0:
        ...
        for chat_id, question in questions:
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton(
                'Ответить', callback_data=question_cb.new(chat_id=chat_id, action='answer')))
...
async def process_answer(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.message.answer('Напишите ответ.', reply_markup=ReplyKeyboardRemove())
